...
class MisboletasexcepcionesController extends ControllerBase
{
    ...
    public function enviomensajeAction()
    {
        ...
        $ruta = 'http://localhost:8010/controlexcepcionesvistobueno/vistobueno/';
        if (is_object($param)) {
            $ruta = 'http://' . $param->nivel . '/controlexcepcionesvistobueno/vistobueno/';
        }

        /**
         * La operación de solicitud puede ser producto de una previa VERIFICACION
         */
                ...
                if ($controlexcepcion->controlexcepcion_estado == 1
                    ...
                    if ($idRelaboralDestinatarioPrincipal > 0) {
                        ...
                        if ($userMail != '' && $passMail != '' && $hostMail != '' && $portMail != '') {
                            ...
                            /**
                             * Se establece el registro del historial de envío de mensajes por correo electrónico.
                             * La posibilidad de modificar registro sólo se da cuando el estado  del registro es positivo.
                             */
                            ...
                            $mail->Subject = ucwords(utf8_decode("SOLICITUD " . $operacionSolicitada . " DE EXCEPCIÓN"));
                            $mail->MsgHTML($cuerpo);
                            $mail->AddAddress($contactoDestinatarioPrincipal->e_mail_inst, $relaboralDestinatarioPrincipal->nombres);
                            $mail->AddCC($userMail, "ali.nelson");
                            $mail->IsHTML(true);
                            $mail->smtpConnect([
                                'ssl' => [
                                    'verify_peer' => false,
                            ...
                            if ($mail->Send()) {
                                ...
                                $mailCopia->AddCC($userMail, "ali.nelson");
                                /**
    ...
    /**
     * Función para la validación de la temporalidad de una solicitud.
     */
    public function verificavalideztemporalidadsolicitudAction()
    {
        ...
        if ($plazo >= 0) {
            $msj = array('result' => 1, 'cantidad' => $plazo, 'msj' => '&Eacute;xito: el registro es válido.');
        } else {
            $plazo = $plazo * -1;
            if ($opcion == 1) {
