...
class OrganigramasController extends ControllerBase
{
	...
	public function indexAction($resolucion_ministerial_id=0)
	{
        if ($resolucion_ministerial_id!=0) {
            ...
            $resolucion_ministerial = Resoluciones::findFirst(array("(uso=1 or uso = 3) and activo=1 and baja_logica=1"));
            if (is_object($resolucion_ministerial)) {
                ...
                if(is_object($organigrama)){
                    $this->listar($organigrama->id,$organigrama->unidad_administrativa, $organigrama->sigla,$organigrama->resolucion_ministerial_id);
                }
            }else{
                $resolucion_ministerial = Resoluciones::findFirst(array("(uso=1 or uso = 3) and baja_logica=1", 'order'=>' id DESC', 'limit'=>'1'));
	...
	public function editAction($id='')
	{
		...
		if ($this->request->isPost()) {
			...
			if ($resul->save()) {
			    /*
			     * Agregado para el control de la glosa permanente
			     */
                $db = $this->getDI()->get('db');
                /*$ok = $db->execute("SELECT * FROM f_organigramas_aplica_glosa_permanente()");*/
				$this->flashSession->success("Exito: Registro guardado correctamente...");
			}else{
				$this->flashSession->error("Error: no se guardo el registro...");
			}
